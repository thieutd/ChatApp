                                            ...
                                            const WebSocketMessageType &type)
{
    ...
    Json::Value json;
    std::string errs;
    if (std::istringstream is(std::move(message)); !Json::parseFromStream(reader, is, &json, &errs) || !json.isObject())
    {
        wsConnPtr->send("Invalid JSON");
        return;
    }
    ...
    std::string msg_id = json["message_id"].asString();
    if (msg.empty())
    {
        wsConnPtr->send("Empty message");
        return;
    }

    auto filter_current = [wsConnPtr](const auto &pair) { return pair.second != wsConnPtr; };

    auto websocket_conns = websocket_connections_ | std::views::values |
                           std::views::transform([](const auto &inner_map) { return inner_map | std::views::all; }) |
                           std::views::join | std::views::filter(filter_current);

    Json::Value ret = Json::objectValue;
    ret["type"] = "message";
    ret["message"] = msg;
    ret["sender_id"] = user_id;
    std::string ret_str = Json::writeString(Json::StreamWriterBuilder(), ret);

    for (const auto &conn : websocket_conns | std::views::values)
    {
        conn->send(ret_str);
    }
