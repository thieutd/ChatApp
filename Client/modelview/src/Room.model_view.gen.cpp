...
Room_model::Room_model(QObject * parent /* = 0 */) : Room_model_base_class(parent) { ; }

Room_model::Room_model(qx::IxModel * other, QObject * parent) : Room_model_base_class(other, parent) { ; }

Room_model::~Room_model() { ; }

QObject * Room_model::list_of_room_membership(int row, bool bLoadFromDatabase /* = false */, const QString & sAppendRelations /* = QString() */)
{
   ...
   Room_model_base_class::type_ptr ptr = this->getRowItemAt(row);
   if (! ptr) { qAssert(false); return NULL; }
   quint64 id = ptr->getid();
   Room::type_list_of_room_membership value = ptr->getlist_of_room_membership();
...
QObject * Room_model::list_of_message(int row, bool bLoadFromDatabase /* = false */, const QString & sAppendRelations /* = QString() */)
{
   ...
   if (! ptr) { qAssert(false); return NULL; }
   quint64 id = ptr->getid();
   Room::type_list_of_message value = ptr->getlist_of_message();
...
void Room_model::syncNestedModel(int row, const QStringList & relation)
{
   Q_UNUSED(relation);
   ...
   if (pNestedModel)
   {
      ...
      Room::type_list_of_room_membership value;
   ...
   {
      this->syncNestedModelRecursive(pNestedModel, relation);
      Room::type_list_of_message value;
      qx::model_view::sync_nested_model(pNestedModel, value);
      ptr->setlist_of_message(value);
   }
}

void Room_model::syncAllNestedModel(const QStringList & relation)
{
